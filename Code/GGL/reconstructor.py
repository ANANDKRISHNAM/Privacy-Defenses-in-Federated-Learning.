    ...
    def ng_loss(z, # latent variable to be optimized
        ...
        if defense_setting is not None:
            ...
            if 'representation' in defense_setting: # for ResNet
                mask = input_gradient[-2][0]!=0
                trial_gradient[-2] = trial_gradient[-2] * mask
            if 'pruning' in defense_setting:
                percentage = 10
                input_gradient = defense.gradient_pruning(input_gradient, percentage=percentage)
                print('Gradient pruning applied with percentage: {}'.format(percentage))
            if 'quantize_gradient' in defense_setting: 
                trial_gradient = defense.quantize_gradient(input_gradient, num_bits=8)

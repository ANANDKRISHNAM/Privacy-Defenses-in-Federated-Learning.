...
def gradient_pruning(input_gradient, percentage=10):
    """
    Prune by percentage
    """
    ...
    for i in range(len(input_gradient)):
        ...
        num_params = flattened_weights.shape[0]
        num_pruned = int(num_params * (percentage/100))
        if num_pruned > 0:
            idxs = np.argpartition(flattened_weights, num_pruned)[:num_pruned]
            grad_tensor = np.where(np.isin(grad_tensor, flattened_weights[idxs]), 0, grad_tensor)
